...
export async function fbGetCampaigns(access_token, accntIds, int_method, int_base, spinnerUpdate) {
    let accntData = [];
    let fbReturn = {};
    let spinnerInterval = int_method / accntIds.length;
    for (let idx = 0; idx < accntIds.length; idx++) {
        spinnerUpdate((idx + 1) * spinnerInterval + int_base);
...
export async function fbGetAdSets(access_token, itemIds, int_method, int_base, spinnerUpdate) {
    let itemData = [];
    let fbReturn = {};
    let spinnerInterval = int_method / itemIds.length;
    for (let idx = 0; idx < itemIds.length; idx++) {
        spinnerUpdate((idx + 1) * spinnerInterval + int_base);

        // console.log('[spinnerUpdate][spinner-interval]', (idx + 1) * spinnerInterval + int_base);
        await RandomSleep(1, 3);
...
export async function fbGetAds(access_token, itemIds, int_method, int_base, spinnerUpdate) {
    let itemData = [];
    let fbReturn = {};
    let spinnerInterval = int_method / itemIds.length;
    for (let idx = 0; idx < itemIds.length; idx++) {
...
export async function fbGetAdCreatives(access_token, itemIds, int_method, int_base, spinnerUpdate) {
    let itemData = [];
    let fbReturn = {};
    let spinnerInterval = int_method / itemIds.length;
    for (let idx = 0; idx < itemIds.length; idx++) {
...
const image_fields = "account_id,created_time,width,url_128,url,updated_time,status,original_width,original_height,name,is_associated_creatives_in_adgroups,id,height,hash,permalink_url,creatives";
var image_base_url = '~item_id~/adimages?access_token=~access_token~&fields=' + image_fields;
export async function fbGetAdImages(access_token, itemIds, int_method, int_base, spinnerUpdate) {
    let itemData = [];
    let fbReturn = {};
    let spinnerInterval = int_method / itemIds.length;
    for (let idx = 0; idx < itemIds.length; idx++) {
        ...
        await RandomSleep(1, 3);
        const itemId = itemIds[idx];
        console.log('[fbGetAdImages] Getting item id ' + itemId + '...');
        let url = image_base_url.replace('~access_token~', access_token)
            .replace('~item_id~', 'act_' + itemId);
